...
import "../Kendra Holders/kendra-kyc.css";
...
function KendraKycform() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const url = `${BASE_URL}/api/kyckendra/create`;
        const config = {
            headers: {
        ...
        if (isChecked) {
            try {
                ...
                alert(`User KYC uploaded successfully with ID: ${response.data.object.userId}`);
                // Handle success (e.g., clear form fields)
                const trxRes=await axios.post(`${BASE_URL}/api/transactions`,{kyc_status:"active"});
                console.log(trxRes.data);
                ...
                Navigate("/crm/user/kendra-kyc-form/preview-form")
            } catch (error) {   
    ...
    return (
        <Fragment>
            <h1 className='text-center text-white fs-2' style={{ paddingTop: "10rem", height: "15rem" }}>Kendra KYC FORM</h1>
            <br />
